...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/05/03 12:47:14 by fnancy            #+#    #+#             */
/*   Updated: 2021/05/03 14:46:38 by fnancy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_archetecture_32(int i, char *filename)
{
	write(1, "\n", 1);
...
int	handle_fat32(void *ptr, char *filename, off_t size)
{
	...
	while (++i < nfat)
	{
		...
		if (nfat > 1 && !is_hostarch(swap_uint32(arch[i].cputype)))
			print_archetecture_32(i, filename);
		ft_nm(ptr + swap_uint32(arch[i].offset), filename, size, 0);
